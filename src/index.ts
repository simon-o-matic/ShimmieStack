...
>({
    ...
    try {
        ...
        if (lastHandledSequenceNumber) {
            ...
            lastHandledSequenceNumber =
                lastHandledSequenceNumber < dbMaxSeqNum
                    ? lastHandledSequenceNumber
                    : dbMaxSeqNum
        }
...
>(
    ...
    const funcs: StackType<RecordModels, SubscribeModels> = {
        ...
        ensureMinSequenceNumberHandled: async ({
            ...
            const requiredSeqNumForReplay =
                minSequenceNumber ??
                (await eventStore.getLatestDbSequenceNumber())
            const lastHandledSeqNum = eventStore.getLastHandledSeqNum()

            // replay any events after the last handled, if the requested minimum > last handled. If no requested min, get db max.
            if (lastHandledSeqNum < requiredSeqNumForReplay) {
                Logger.debug(
                    `Behind requested requested minSeqNum. Executing minSeqNum lookup: ${JSON.stringify(
                        {
                            minSeqNum: requiredSeqNumForReplay,
                            lastHandledSeqNum: lastHandledSeqNum,
                        }
                    )}`
                )
                await eventStore.replayEvents(lastHandledSeqNum + 1) // +1 so we don't replay if both on the same max event
            }
        ...
        getLastHandledSequenceNumber: () => eventStore.getLastHandledSeqNum(),
